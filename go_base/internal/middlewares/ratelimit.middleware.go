...
func (rl *RateLimiter) filterLimitUrlPath(urlPath string) *limiter.Limiter {
	urlPathMap := map[string]*limiter.Limiter{
		"/v1/2024/user/login": rl.publicAPIRateLimiter,
		"/ping/80":            rl.publicAPIRateLimiter,
		"/v1/2024/user/info":  rl.userPrivateAPIRateLimiter,
		"/ping/50":            rl.userPrivateAPIRateLimiter,
	}

	if limiter, exists := urlPathMap[urlPath]; exists {
		return limiter
	}
